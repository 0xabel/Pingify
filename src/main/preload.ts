...
const electronHandler = {
	...
	getAppData: () => ipcRenderer.invoke(ipcChannels.GET_APP_DATA),
	setAppData: (data: any) => ipcRenderer.send(ipcChannels.SET_APP_DATA, data),
	deleteTask: (id: string) => ipcRenderer.send(ipcChannels.DELETE_TASK, id),
	deleteSelectedTasks: (ids: string[]) =>
		ipcRenderer.send(ipcChannels.DELETE_SELECTED_TASKS, ids),

	getProxyPing: (id: string) =>
		ipcRenderer.invoke(ipcChannels.GET_PROXY_PING, id),
	startSelectedTasks: (ids: any) =>
		ipcRenderer.invoke(ipcChannels.START_SELECTED_TASKS, ids),
	setAppSettings: (data: any) =>
		ipcRenderer.send(ipcChannels.SET_APP_SETTINGS, data),
	getAppSettings: () => ipcRenderer.invoke(ipcChannels.GET_APP_SETTINGS),
